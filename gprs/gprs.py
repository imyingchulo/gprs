...
class GPRS( object ):
                    ...
                    for plink_files in os.listdir( self.plink_bfiles_dir() ):
                        if "{}_{}.bed".format( chrnb, plink_bfile_name ) in plink_files:
                                        ...
                                        os.system( "{} --bfile {}/{} --clump {}/{} --clump-p1 {} --clump-p2 {} --clump-r2 {} --clump-kb {} --clump-field {} --clump-snp-field {} --out {}/{}_{}".format(
                                                ...
                                                self.plink_clump_dir(), chrnb, output_name_with_conditions) )
    ...
    def select_clump_snps(self, qc_file_name, clump_file_name, output_name, clump_kb, clump_p1, clump_r2 ):
        clump_conditions = "{}_{}_{}".format(clump_kb, clump_p1, clump_r2 )
        for nb in range( 1, 23 ):
            chrnb = "chr{}".format( nb )
            #extract clumped snplist
            try:
                df = pd.read_csv( "{}/{}_{}_{}.clumped".format( self.plink_clump_dir(), chrnb, clump_file_name, clump_conditions ) , delim_whitespace=True )
                new_df = df.loc[(df['CHR'] == nb)]
                new_df_2 = new_df[['CHR', 'SNP']]
                new_df_2.to_csv("{}/{}_{}_{}_clumped_snplist.csv".format( self.plink_clump_dir(), chrnb, clump_file_name, clump_conditions ), sep=' ', index=False, header=True )
            except IOError:
                print("{}/{}_{}_{}.clumped not found. skip".format( self.plink_clump_dir(), chrnb,clump_file_name, clump_conditions ))
            #generate snplist by comparing clumped/qced snplists
            try:
                clump_snp = pd.read_csv("{}/{}_{}_clumped_snplist.csv".format( self.plink_clump_dir(), chrnb,clump_file_name, clump_conditions ), sep=' ' )
                ...
                if os.path.exists(qc_files1):
                    ...
                    newsnplist.to_csv( "{}/{}_{}_{}.qc_clump_snpslist.csv".format( self.qc_clump_snpslist_dir(), chrnb, output_name, clump_conditions ), sep=' ', index=False, header=True )
                ...
                print("{}/{}_{}_{}_clumped_snplist.csv not found. skip".format( self.plink_clump_dir(), chrnb, clump_file_name, clump_conditions))
        print( "All jobs are completed" )

    def build_prs(self, vcf_input, output_name, qc_file_name, memory, clump_kb, clump_p1, clump_r2, symbol='.' ,columns='1 2 3', plink_modifier='no-mean-imputation'):
        clump_conditions = "{}_{}_{}".format(clump_kb, clump_p1, clump_r2 )
        for nb in range( 1, 23 ):
            chrnb = "chr{}".format( nb )
            for vcf_file in os.listdir( vcf_input ):
                if vcf_file.endswith('.vcf.gz') and chrnb != "chrY" and chrnb != "chrX" and chrnb != "wgs" and "{}{}".format(chrnb, symbol)in vcf_file:
                    qc_file = "{}/{}_{}_{}.qc_clump_snpslist.csv".format( self.qc_clump_snpslist_dir(), chrnb, qc_file_name, clump_conditions )
                    try:
                        os.path.exists(qc_file)
                        os.system("plink2 --vcf {}/{} dosage=DS --score {} {} '{}' --memory {} --out {}/{}_{}_{}".format( vcf_input, vcf_file,
                                                                                                 qc_file, columns,
                        ...
                        print( "{}/{}_{}_{}.qc_clump_snpslist.csv not found. skip".format( self.qc_clump_snpslist_dir(), chrnb, qc_file_name, clump_conditions ) )
    ...
    def prs_statistics(self, score_file, pheno_file, output_name, data_set_name, prs_stats_R, r_command, clump_kb, clump_p1, clump_r2 ):
        ...
        call("{0} --vanilla {1} {2} {3} {4} {5} {7}/{6}".format(r_command, prs_stats_R, score_file, pheno_file,
                                        data_set_name, filter_condition, output_name, self.stat_dir()), shell=True)
    ...
    def combine_prs_stat(self,data_set_name, clump_kb, clump_p1, clump_r2):
        filter_condition = "{}_{}_{}".format(clump_kb, clump_p1, clump_r2 )
        interesting_files = glob.glob( "{}/{}_{}*.txt".format(self.stat_dir(), data_set_name, filter_condition) )
        combined_csv = pd.concat( [pd.read_csv( f ) for f in sorted( interesting_files )] )
        combined_csv.to_csv(
            '{}/{}_{}_combined_stat.txt'.format( self.stat_dir(), data_set_name, filter_condition ),
            index=False,
            sep='\t'
        )