...
@click.option( '--output_name', metavar='<str>', default='gwas', help='output name; default: gwas ; the output file will be named as [chrnb]_[output_name].csv' )
...
def generate_plink_bfiles(ref, result_dir, output_name):
    gprs = GPRS( ref=ref, result_dir=result_dir )
    gprs.generate_plink_bfiles( output_name=output_name )
...
@click.option( '--output_name', metavar='<str>', required=True, help='output name should remain consistent as output_name to plink and filtered data; output format: [chrnb]_[output_name]_clumped_snplist.csv' )
...
def clump(ref, data_dir, result_dir, clump_kb, clump_p1, clump_p2, output_name, clump_r2, clump_field, clump_snp_field):
...
@click.option( '--ref', metavar='<str>', help='path to population reference panel' )
@click.option( '--result_dir', metavar='<str>', default='./result', help='path to output folder, default: "./result"' )
@click.option( '--output_name', metavar='<str>', required=True, help='output name should remain consistent as output_name to plink and filtered data; output format is: [chrnb]_[output_name]_clumped_snplist.csv' )
def select_clump_snps(ref, result_dir, output_name):
    gprs = GPRS( ref=ref, result_dir=result_dir )
